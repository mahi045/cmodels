...
Output::print_wfm(){
  ...
  cout<<"\nComputeTrue\n";
  for (long i=0;i<program->number_of_atoms;i++)	 
	if((program->atoms[i]->computeTrue||program->atoms[i]->computeTrue0) && program->atoms[i]->Bpos==false){
	  if(strcmp("#noname#",	program->atoms[i]->atom_name())){
		printf(":-not %s .",	program->atoms[i]->atom_name ());
...
Output::PrintStats(){
  if(!stat)
	return;
  cout<<"Stats #a "<<program->number_of_atoms<<
	"&#cl "<< program->number_of_clauses<<
    "&#r "<< numRules<<
    "&#sr "<< program->number_of_rules<<
    "&#nr "<< program->number_of_nestedRules;
  if(param->sort)
